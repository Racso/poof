...
- `poof add <name> --external <container>[:<port>]` — **external project**: Poof owns the domain and routing, nothing else. No image, repo, branch or CI; Caddy proxies straight to a container Poof doesn't manage (Compose, hand-run). Port defaults to 80. Registration **refuses if the container doesn't exist** (existing is enough — a stopped container still proves the name is real), so a typo fails immediately instead of surfacing later as a 502. The per-app network is created and the upstream attached at registration, since there's no deploy step to do it. `deploy`/`rollback`/`snapshot` refuse; `remove` deletes the route and network and explicitly reports that the container was left running. `list`/`status` report the status `external` and print only what Poof actually knows (domain + upstream) — the container is not Poof's, so there is no running state to report and `projectRunning` never inspects one. Replaces the static-project-plus-snippet shim.
...
- `poof net create|ls|delete|add|show|list|remove` — Poof-managed Docker networks for deliberate connectivity. `create <name> [--internal]` defines a network; `add <network> [member...] [--caddy] [--poof]` attaches members; `show <network>` lists them; `list <project>` shows a project's networks; `remove <network> [member...]` detaches. `delete` refuses while anything is attached and leaves the underlying Docker network in place.
  ...
  - **`--caddy` is reachability, not routing.** It attaches the Caddy container so it *can* dial the members; the Caddyfile is generated from projects, redirects and snippets only, so `net add --caddy` contributes zero bytes to it and no domain points anywhere. Routing a domain to a non-Poof container is `poof add <name> --external <container>` (route + network + attachment in one command); `--caddy` is for the case where the route is published by hand with `poof caddy set`.
  - **A project's own per-app network is a valid attachment point** (`appNetworkOwner`, `server/networks.go`). `poof-app-<project>` is absent from the `networks` table — it is created at deploy time — but `addNetworkMembers` accepts it when the project exists and is not static. That is where an unmanaged container has to live to be reachable as a `poof spell proxy` target, and recording the attachment instead of `docker network connect`-ing it is what makes it survive a recreate. Project removal deletes every member of `poof-app-<name>` along with the network: a stale row would have reconciliation recreate the network on the next sync.
  - **An external project's upstream is recorded as a member** of `poof-app-<name>` (`ensureProjectNetwork`), and backfilled for projects registered before that (`ensureExternalMemberships`, run at the top of every reconcile — no schema migration). Without the row it was the one attachment nothing re-applied: Poof never deploys that container, so a `docker compose down && up` detached it and the project 502'd permanently.
...
- `poof spell proxy <source> <target>` — install a Caddy reverse_proxy on a source project. Source: `<project>` or `<project>/<path>`. Target: `<project>` (port resolved from DB) or `<container>:<port>`. Strip-prefix implicit when path is present; `--keep-prefix` opts out. A raw `<container>:<port>` target is also **attached** to `poof-app-<source>` and recorded as a member, since a snippet only names an upstream — it doesn't make it reachable. A static source has no network of its own, so the spell prints the `poof net create` / `net add --caddy` recipe instead.
...
- `poof-net` is the control plane: Caddy + the Poof daemon only, **enforced** — `server.ControlPlaneNetwork`; `poof net create poof-net` and `poof net add poof-net ...` are both refused. A convention would erode one exception at a time, and every exception becomes a neighbour of the daemon (which holds the Docker socket). Anyone wanting a shared network creates their own and attaches Caddy/the daemon with `--caddy` / `--poof`.
- Cross-project traffic is always deliberate: a Poof-managed extra network (`poof net create` + `net add`), or `poof net add poof-app-<name> <container>` to invite a hand-managed container into a project's world (e.g. as a raw `poof spell proxy` target — which does exactly that call). A manual `docker network connect` still works but is not reconciled, so it is lost the moment the container is recreated.
...
- **Every routing change is verified, and a failed sync is reported.** `poof caddy set` adapt-checks the candidate config (`POST /adapt`) before storing the snippet — the project is force-routed for the check, since snippets are usually set before the first deploy — and a rejected snippet 400s without being written. `caddy.ErrValidateUnavailable` (admin API unreachable) 503s rather than passing — unverifiable is not valid, and storing it would push an unchecked snippet on the next sync. `poof caddy set --no-validate` (`skip_validate` in the body) skips the adapt check entirely and is the escape hatch for a host whose Caddy is down. When a reload does fail, `syncRouting` logs at ERROR, sets the `X-Poof-Routing-Error` response header and injects `routing_error` into the JSON body; the CLI prints a banner and exits non-zero. The stored change is kept — the DB is desired state — but nothing reports success.
