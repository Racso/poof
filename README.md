...
A snippet is checked before it is stored: Poof! renders the config it would produce and asks Caddy to adapt it, so a syntax error comes back as an error on the command instead of a stored snippet that breaks the next reload. If Caddy cannot be reached the snippet is **not** stored either — unverifiable is not the same as valid. Pass `--no-validate` to skip the check and store it regardless.
...
- **Auth or connection errors from the CLI?** Run `poof troubleshoot` — it probes config, DNS, TCP, TLS, HTTP and auth in order and names the first thing that fails. With multiple servers, make sure the right profile is active (`--profile <name>` or `POOF_PROFILE` + `--profile-env`).
