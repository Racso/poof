...
git push
  ...
  → GitHub Action: POST /projects/:name/deploy  (image tag + per-project token)
...
There are no GitHub webhooks involved. The GH Action is the trigger — it calls Poof! directly after the build, because it's the only one that knows the image tag. A webhook would fire before the build completes and would have no image tag to pass.
...
poof add demo
  ...
  → PUT  /repos/racso/demo/actions/secrets/POOF_URL
      stores the server's public address as a repo secret
...
No webhook creation. The developer never manually sets secrets or copies workflow files.
...
POST   /projects/:name/deploy     trigger deploy (called by GH Action or manually; per-project token)
...
poof/
  ...
  github/
    client.go           GitHub API: secrets, workflow file commit
  store/
...
**Why per-project tokens instead of a global token?**
...
Zero manual steps for the developer. `poof add` is the only thing that needs to happen. The committed workflow is also Poof!-version-aware: if the API changes, `poof add` (or a future `poof upgrade`) can update it.
